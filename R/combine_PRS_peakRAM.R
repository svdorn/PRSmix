...
#' @importFrom utils head read.table peakRAM
#' @export
combine_PRS_peakRAM = function(
  pheno_file,
  covariate_file = NULL,
  score_files_list,
  ...
  for (power_thres in power_thres_list) {
    for (pval_thres in pval_thres_list) {
      ...
      write.table(ram, paste0(out, "_power.", power_thres, "_pthres.", pval_thres, "_peakRAM_PRSmix.txt"),
                         row.names = FALSE, sep = "\t", quote = FALSE)
