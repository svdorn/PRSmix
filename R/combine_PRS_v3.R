...
combine_PRS_v3 = function(
	...
	if (!read_pred_training_1) {
		...
		pred_acc_train_allPGS_summary = eval_multiple_PRS_v3(train_df, pgs_list_all, covar_list, liabilityR2, alpha=0.05, isbinary=isbinary, ncores=ncores)
	...
	bestPRS_acc = eval_single_PRS_v3(test_df, pheno = "trait", prs_name=bestPRS, covar_list=covar_list, liabilityR2, alpha=0.05, isbinary=isbinary)
	...
	if (!read_pred_testing_1) {
		...
		pred_acc_test_trait = eval_multiple_PRS_v3(test_df, pgs_list,  covar_list, liabilityR2, alpha=0.05, isbinary=isbinary, ncores=ncores)
		...
		for (pval_thres in pval_thres_list) {
			...
			if (length(topprs) == 0) {
				...
				if (!isbinary) {
					...
					res_lm1 = eval_single_PRS_v3(test_df1, pheno = "trait", prs_name="newprs", covar_list=covar_list, liabilityR2 = liabilityR2, alpha=pval_thres, isbinary=isbinary)
			...
			if (length(topprs) == 0) {
				...
				if (!isbinary) {
					...
					test_df1 = cbind(test_data, IID=test_df$IID)
					test_df1$newprs = as.matrix(test_df1[,topprs]) %*% as.vector(ww)
					res_lm = eval_single_PRS_v3(test_df1, pheno="trait", prs_name="newprs", covar_list=covar_list, liabilityR2 = liabilityR2, alpha=pval_thres, isbinary=isbinary)
					res_lm$pgs = "PRSmix+"
					res_lm
					...
					test_df1 = cbind(test_data, IID=test_df$IID)
					test_df1$newprs = as.matrix(test_df1[,topprs]) %*% as.vector(ww)
					res_lm = eval_single_PRS_v3(test_df1, pheno="trait", "newprs", covar_list, liabilityR2 = liabilityR2, alpha=pval_thres, isbinary=isbinary)
					res_lm$pgs = "PRSmix+"
					res_lm
